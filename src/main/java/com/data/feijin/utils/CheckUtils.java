...
public class CheckUtils {
    ...
    public static void match2(){
        ...
        while (i<list.size()){
            String k = list.get(i).trim();
            Map<String,String> map = new HashMap();
            if(i<=list.size() && k.equals("Score")){
                map.put(list.get(i).trim(),list.get(++i).trim());
                String g = list.get(++i).trim();
                if(i<=list.size() && g.equals("GroupName")){
                    map.put(g,list.get(++i).trim());
                }
            }else {
                map.put(list.get(i).trim(),list.get(++i).trim());
            }
        ...
        lResult.forEach(l->System.err.println(l));
    }


    private Map parseGroupData(List<String> list){
        int i = 0;
        while (i<list.size()){
            String k = list.get(i).trim();
            Map<String,String> map = new HashMap();
                ...
                if(null != value){
                  String[] sVal =   value.split("|");
                    ...
                    for (int j=0;j<sVal.length;j++) {
                        ...
                        color.append(data[2]).append("|").append(groupName).append("|").append(groupNum);
                        sMap.put("relegation",color.toString());
                        sMap.put("rank",data[0]);
                        sMap.put("teamId",data[1]);
                        sMap.put("matchs",data[6]);
                        sMap.put("win",data[7]);
                        sMap.put("draw",data[8]);
                        sMap.put("loss",data[9]);
                        sMap.put("score",data[10]);
                        sMap.put("lossScore",data[11]);
                        sMap.put("goalDifference",data[12]);
                        sMap.put("lossPointPerGame",o.get(15));
                        sMap.put("totalPoints",o.get(16));
                        sMap.put("deductPoints",o.get(17));
                        sMap.put("deductionDescription",o.get(18));
                        sMap.put("situation",o.get(19)+","+o.get(20)+","+o.get(21)+","+o.get(22)+","+o.get(23)+","+o.get(24));
                        sMap.put("season",season);
                        sMap.put("league",leagueId);
                    }
                }
                String g = list.get(++i).trim();
                if(i<=list.size() && g.equals("GroupName")){
